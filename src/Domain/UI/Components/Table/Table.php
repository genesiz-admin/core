...
class Table
{
    ...
    public function resolveRow($row)
    {
        ...
        foreach ($this->columns as $col) {
            ...
            $tempRow[$col->getKey()] = $col->resolveValue($row);
            // if ($col->getFormatter()) {
            //     $tempRow[$col->getKey()] = call_user_func($col->getFormatter(), $row[$col->getKey()], $row);
            // }
    ...
    {
        $rows = $this->isPaginated() ? $this->query->items() : $this->query->all();

        $output =  [
            'data' => array_map(fn ($row) => $this->resolveRow($row), $rows),
